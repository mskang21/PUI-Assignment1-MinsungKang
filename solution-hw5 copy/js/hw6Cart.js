...
function submitNote() {
    ...
    rollEditorImage.src = "./images/" + roll.type + "-cinnamon-roll.jpg";
    const editorRollImage = rollEditorImage.src;
  
    const rollEditorType = document.querySelector('#roll-type');
    const editorRollType = rollEditorType.value;
  
    const rollGlazing = document.querySelector('#glazing');
    const editorRollGlazing = rollGlazing.value;

    const rollPackSize = document.querySelector('#pack-size');
    const editorRollPackSize = rollPackSize.value;

    const rollPrice = document.querySelector('#pricing');
    const editorRollPrice = rollPrice.value;

  
    const roll = addNewNote(editorRollImage, editorRollType, editorRollGlazing, editorRollPackSize, editorRollPrice);
    createElement(roll);
...
function saveToLocalStorage() {
    ...
    localStorage.setItem('storedNotes', rollArrayString);
}

function retrieveFromLocalStorage() {
    const rollArrayString = localStorage.getItem('storedNotes');
    const rollArray = JSON.parse(rollArrayString);
    for (const rollData of rollArray) {
      const roll = addNewNote(rollData.rollImageURL, rollData.noteTitle,
        rollData.rollBody);
      createElement(roll);
    }
}
